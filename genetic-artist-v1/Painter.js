  ...
  breed (mate) {
    ...
    for(let i = 0; i < mateGenes.length; i++) {
      ...
      if(Math.random() < mutationRate) {
        ...
        gene.r = Math.abs (gene.r + Math.random () * (MAX_COLOR_SHIFT - MIN_COLOR_SHIFT) + MIN_COLOR_SHIFT);
        gene.g = Math.abs (gene.g + Math.random () * (MAX_COLOR_SHIFT - MIN_COLOR_SHIFT) + MIN_COLOR_SHIFT);
        gene.b = Math.abs (gene.b + Math.random () * (MAX_COLOR_SHIFT - MIN_COLOR_SHIFT) + MIN_COLOR_SHIFT);
        if(gene.r > 255)
          gene.r = 255;
        if(gene.g > 255)
          gene.g = 255;
